...
## 8. 🔍 GitHub PR 자동 코드 리뷰
구독한 저장소에 새 PR이 올라오면 PR 본문에 👀 리액션을 자동 추가합니다.
변경 파일/패치를 비동기로 분석해 인라인 코멘트 + 종합 리뷰를 자동 작성합니다.
리뷰는 반드시 한국어로 작성되며, 실패 시 큐에 저장해 재시도합니다.
PR 리뷰 생성에는 OpenAI gpt-5 계열 모델을 사용합니다.
---
